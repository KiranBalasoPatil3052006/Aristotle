...
public sealed class DatabaseService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation("Authorization attempt for email: {Email}", emailAddress);
        ...
        await using (var connection = await OpenConnectionAsync(readOnly: false, cancellationToken))
        {
            await using (var command = connection.CreateCommand())
            {
                ...
                command.Parameters.AddWithValue("@email", emailAddress.Trim());

                await using var reader = await command.ExecuteReaderAsync(cancellationToken);
                if (!await reader.ReadAsync(cancellationToken))
                {
                    _logger.LogWarning("Authorization failed: no active authorized member matched email: {Email}", emailAddress);
            ...
            {
                _logger.LogWarning("Authorization failed: invalid password for email: {Email}", emailAddress);
                return (false, 0, "", "", "", "Invalid authorization details.");
            }
    ...
    private static async Task SeedAuthorizedMembersAsync(SqliteConnection connection, IDbTransaction transaction, CancellationToken cancellationToken)
    {
        var members = new (int Id, string Name, string RoleName, string Email, string PasswordHash)[]
        {
            ...
            (2, "Arun Manager", "Manager", "arun.manager@company.com", HashPassword("Manager@123")),
            (3, "Rahul Director", "Director", "rahul.director@company.com", HashPassword("Director@123")),
            (4, "CEO Admin", "Admin", "ceo@company.com", HashPassword("Admin@123"))
        };
